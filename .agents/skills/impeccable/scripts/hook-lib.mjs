...
export function resolveHarness(env = {}, event = null) {
  ...
  // Codex's PostToolUse/PreToolUse/SessionStart/etc. wire formats do accept a
  // `hookSpecificOutput` wrapper (same shape Claude Code uses), which is why
  // this used to fold 'codex' into the 'claude' harness outright. Its Stop
  // (and SubagentStop) wire format does NOT: Codex's own hook schema has no
  // "StopHookSpecificOutputWire" counterpart to the PostToolUse/SessionStart
  // ones, only a flat `StopCommandOutputWire`. Sending Stop's
  // `hookSpecificOutput.additionalContext` payload to Codex therefore fails
  // its deserializer with "hook returned invalid stop hook JSON output" --
  // confirmed against the codex-cli binary's own embedded JSON-schema
  // strings, and against the exact error text reported for this repo.
  // `codex` is kept as its own harness value (not re-collapsed into
  // 'claude') so `payload()` can special-case just the Stop event below,
  // without touching the PostToolUse channel that already works for Codex.
  if (explicit === 'codex') return 'codex';
  // No env var currently sets IMPECCABLE_HOOK_HARNESS=codex (the generated
  // .codex/hooks.json command doesn't set it) -- CODEX_SANDBOX /
  // CODEX_SANDBOX_NETWORK_DISABLED are set by the real codex-cli around
  // every command it runs (including hook subprocesses), so they are a
  // reliable auto-detect signal in the absence of the explicit override.
  if (env?.CODEX_SANDBOX !== undefined || env?.CODEX_SANDBOX_NETWORK_DISABLED !== undefined) return 'codex';
...
export function payload(text, eventName = 'PostToolUse', harness = 'claude') {
  // Codex's Stop hook has no `hookSpecificOutput`-wrapped wire format (see
  // resolveHarness() above) -- only Claude Code's Stop hook understands
  // additionalContext-style continuation there. Emitting nothing is the
  // same safe no-op every "skipped"/"clean" Stop result already returns
  // (empty stdout, exit 0), so this silently drops the deep-pass summary
  // for Codex specifically rather than crashing the turn with an invalid
  // JSON output error. Codex's PostToolUse channel is unaffected: it keeps
  // its own HookSpecificOutputWire and still gets additionalContext below.

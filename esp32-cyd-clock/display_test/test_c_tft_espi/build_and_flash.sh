...
if [[ "${1:-}" == "--flash" ]]; then
  ...
  "$ESPTOOL" --chip esp32 --port /dev/ttyUSB0 --baud 460800 write-flash -z \
    ...
    0x1000 build_output/test_c_tft_espi.ino.bootloader.bin \
    0x8000 build_output/test_c_tft_espi.ino.partitions.bin \
    0x10000 build_output/test_c_tft_espi.ino.bin
fi
